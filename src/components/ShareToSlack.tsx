...
function buildSlackMessage(data: ProjectHealth): string {
  ...
  lines.push(`${healthEmoji} *<${project.url}|${project.name}>* (${project.key})`);

  const meta: string[] = [];
  if (project.lead) meta.push(`Lead: ${project.lead}`);
  ...
  if (qualitativeHealth && qualitativeHealth.signals.length > 0) {
    lines.push("");
    lines.push(`*Slack signals:* ${qualitativeHealth.summary}`);
    for (const s of qualitativeHealth.signals) {
      lines.push(`  • ${s}`);
    }
  }
  ...
  if (experiments && experiments.length > 0) {
    lines.push("");
    lines.push("*Houston experiments:*");
    for (const exp of experiments) {
      ...
      if (exp.rolloutPercent !== null) detail += ` — ${exp.rolloutPercent}% rollout`;
      if (exp.exposureCount) detail += ` — ${exp.exposureCount.toLocaleString()} exposures`;
      if (exp.srmIssue) detail += " :warning: *SRM issue*";
      lines.push(`  ${detail}`);

      if (exp.metrics && exp.metrics.length > 0) {
        for (const m of exp.metrics.filter((m) => m.isSignificant)) {
          const arrow = m.direction === "positive" ? ":arrow_up:" : ":arrow_down:";
          const effect = m.effectSize !== null ? `${(m.effectSize * 100).toFixed(1)}%` : "?";
          lines.push(`    ${arrow} ${m.metricName}: ${effect} (p=${m.pValue?.toFixed(4)})*`);
        }
      }
    }
  }
...
export function ShareToSlackButton({ data }: { data: ProjectHealth }) {
  ...
  const handleOpen = () => {
    ...
    setSent(false);
    setError(null);
    setMessage(buildSlackMessage(data));
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div ref={modalRef} className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            ...
            <div className="px-4 py-3 border-t flex items-center justify-end gap-2">
              ...
              {!sent && (
                <button
                  onClick={handleSend}
                  disabled={!selectedChannel || !message || sending}
